...
const CountriesCard = ({
  ...
  return (
    <div className="lg:flex lg:relative lg:w-[80%] px-3 pb-[30px] mb-[30px] border-b border-solid border-[#eee]">
      ...
      <div className="countries-content lg:w-[700px] lg:pl-3 relative md:w-[73%] md:mx-auto">
        <h4 className="countries-heading text-[20px] m-0 mb-[5px] text-[#2a2a2a] font-bold">
          {heading}
        </h4>
        <span className="countries-text text-[15px] text-[#afafaf] block">{littleText}</span>
        <div className="absolute right-2 top-0">
          <a
            href="#contact"
            className="md:text-[14px] text-[12px] text-white bg-[#22b3c1] border border-solid border-[#22b3c1] md:py-3 px-4 py-2 md:px-[30px] inline-block rounded-[25px] font-medium capitalize tracking-[0.5px] transition-all duration-[0.3s] hover:opacity-80"
          >
            {t("Explore More")}
          </a>
        </div>
        <p className="countries-txt m-0 my-[18px] text-[15px] leading-[30px] text-[#afafaf]">
          {mainText}
        </p>
